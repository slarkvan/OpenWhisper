...
const modelUrl = "https://huggingface.co/ggerganov/whisper.cpp/resolve/main/ggml-tiny.en.bin?download=true";
...
const download = async (url, dest) => {
  ...
  return axios
    ...
    .then((response) => {
      ...
      response.data.pipe(fs.createWriteStream(dest)).on("close", async () => {
        ...
        if (hash === sha) {
          ...
          console.error(
            chalk.red(
              `❌ Model ${model} not valid, please try again using command \`yarn run download-whisper-model\``
            )
          );
          process.exit(1);
        }
    ...
    .catch((err) => {
      console.error(
        chalk.red(
          `❌ Failed to download ${url}: ${err}.\nPlease try again using command \`yarn run download-whisper-model\``
        )
      );
      process.exit(1);
    });
...
await download(modelUrl, path.join(dir, model));
